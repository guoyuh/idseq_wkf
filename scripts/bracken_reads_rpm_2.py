...
def run(clean_fastq,s_bracken):
    if os.path.exists(s_bracken):
        out = kraken_out + ".csv"
        tb = pd.read_table(kraken_out,sep = "\t")
        in_count = count_clena_reads(clean_fastq)
        print(in_count)
...
if __name__ == "__main__":
    ...
    parser.add_argument('-c','--clean_fq',type=str,help='remove host fastq')
    parser.add_argument('-b','--bracken',type = str ,help='bracken out')
    args = parser.parse_args()
    run(clean_fastq=args.clean_fq,s_bracken=args.bracken)
