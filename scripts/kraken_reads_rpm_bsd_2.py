...
def negative_control(workdir,filter_fq):
            ...
            with open(negative_filter_mpa_report, "r") as fh:
                for line in fh:
                    ...
                    if species_re:
                        ...
                        raw_single = count_reads(fastq=filter_fq)
...
def read_mpa_report(filter_fq,before_filter_mpa_report,filter_mpa_report,workdir,prefix):
    #fungi_organism_name = is_fungi(fungi_assemble_refseq="/mnt/data/kraken2_mask/library/fungi/assembly_summary.txt")
    ...
    tmp_filter_fastq = Path(workdir).glob('result/*/filter/*.filter.fq')
    if len(tmp_filter_fastq) >=1:
        for f in tmp_filter_fastq:
            #print(f.basename())
            if f.find("yinkong") != -1:
                print("find yinkong_filter_fastq======>:", f)
                negative_Species_set = negative_control(workdir=workdir,filter_fq=f)
    #print(genus_cnt)
    total_counts = count_reads(filter_fq)
...
if __name__ == '__main__':
    if len(sys.argv) != 6:
        print("usage: python {} filter_fq  before_filter_mpa_report filter_mpa_report workdir prefix".format(sys.argv[0]))
        print(
            """
            example:\n
            python {} /mnt/home/huanggy/project/20220418/result/21JS944006/filter/21JS944006.filter.fq /mnt/home/huanggy/project/20220418/result/21JS944006/kraken2/21JS944006.kreport.mpa.txt /mnt/home/huanggy/project/20220418/result/21JS944006/kraken2/21JS944006.uniq80_kreport.mpa.txt /mnt/home/huanggy/project/20220418 21JS944006_2
            """.format(sys.argv[0])
              )
