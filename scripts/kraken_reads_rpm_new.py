        ...
        for line in fh:
            ...
            if genus_re:
                ...
                if genus_name not in genus:
                    ...
                    genus[genus_name] = genus_counts
                else:
                    pass
            elif family_re:
                viral_family_name = family_re.group(1).replace("_"," ",2)
                viral_family_counts = int(family_re.group(2))
                if viral_family_name not in genus:
                    genus[viral_family_name] = viral_family_counts
                else:
                    pass
...
def negative_control(negative_filter_mpa_report,single_kneaddata_sum):
    ...
    with open(negative_filter_mpa_report, "r") as fh:
        for line in fh:
            ...
            if species_re:
                ...
                clean_reads = get_rawReads(single_kneaddata_sum=single_kneaddata_sum)
                Species_rpm = "{:.2f}".format(Species_reads / clean_reads * 20 * 1.0e6)
...
def read_mpa_report(single_kneaddata_sum,before_filter_mpa_report,filter_mpa_report,negative_filter_mpa_report,prefix):
    fungi_organism_name = is_fungi(fungi_assemble_refseq="/mnt/data/kraken2_mask/library/fungi/assembly_summary.txt")
    ...
    genus = get_Genus_counts(filter_mpa_report)
    negative_Species_set = negative_control(negative_filter_mpa_report=negative_filter_mpa_report,single_kneaddata_sum=single_kneaddata_sum)
    #print(genus)
    ...
    sample_name=prefix
    header = ["#sample","category","category_reads","Genus_name","Genus_reads","G_rpm","G_relative_Abundance","Latin_name","Latin_reads","lineage","S_relative_Abundance","RP20M","Coverage(ratio)","Depth","BSD-conclusion","FoldChange(BSD)"]
...
if __name__ == '__main__':
    if len(sys.argv) != 6:
        print("usage: python {} single_kneaddata_sum  before_filter_mpa_report filter_mpa_report negative_filter_mpa_report prefix".format(sys.argv[0]))
        print(
            """
            example:\n
            python  ~/idseq_wkf/scripts/kraken_reads_rpm_new.py /mnt/home/huanggy/project/20220408/result/21JS944006/filter/kneaddata.sum.txt /mnt/home/huanggy/project/20220408/result/21JS944006/kraken2/21JS944006.report.mpa.txt /mnt/home/huanggy/project/20220408/result/21JS944006/kraken2/21JS944006.uniq80_kreport.mpa.txt /mnt/home/huanggy/project/20220408/result/21JS944007/kraken2/21JS944007.report.mpa.txt 21JS944006_2
            """
              )
