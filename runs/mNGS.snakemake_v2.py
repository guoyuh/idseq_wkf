...
df = pd.read_csv(config['sample_se_list'],sep = "\t",header=0)
SAMPLES = df["sample_name"].tolist()


# SAMPLES=["S200026380_L01_90","S200023932_L01_97"]
# OUTDIR="/mnt/home/huanggy/project/20211128"
...
rule all:
    ####想要哪一步的结果,都在这里买定义
    input:
        expand("{outdir}/result/filter/{sample}.f.fastq",outdir=OUTDIR,sample=SAMPLES),
        expand("{outdir}/result/filter/{sample}.log",outdir=OUTDIR,sample=SAMPLES),
        expand("{outdir}/result/kraken2/{sample}.report",outdir=OUTDIR,sample=SAMPLES),
        expand("{outdir}/result/kraken2/{sample}.s.bracken",outdir=OUTDIR,sample=SAMPLES),
        expand("{outdir}/result/kraken2/{sample}.s.bracken.csv",outdir=OUTDIR,sample=SAMPLES),
        expand("{outdir}/result/kraken2/{sample}.report.mpa.txt",outdir=OUTDIR,sample=SAMPLES),
        expand("{outdir}/result/kraken2/{sample}.report.mpa.rpm.csv",outdir=OUTDIR,sample=SAMPLES),
        expand("{outdir}/result/kraken2/{sample}.krona",outdir=OUTDIR,sample=SAMPLES),
        #expand("{outdir}/result/align/{sample}.bowtie2.refseq.log",outdir=OUTDIR,sample=SAMPLES)
        expand("{outdir}/result/align/{sample}.refseq.sam",outdir=OUTDIR,sample=SAMPLES),
        expand("{outdir}/result/align/{sample}.bam",outdir=OUTDIR,sample=SAMPLES),
        expand("{outdir}/result/align/{sample}.sorted.bam",outdir=OUTDIR,sample=SAMPLES),
        expand("{outdir}/result/align/{sample}.cove.tsv",outdir=OUTDIR,sample=SAMPLES),
        expand("{outdir}/result/align/{sample}.refseq.cove.tsv",outdir=OUTDIR,sample=SAMPLES),
        #directory(expand("{outdir}/result/filter/spades_out",outdir=OUTDIR,sample=SAMPLES)),
        expand("{outdir}/result/spades/{sample}/spades_out/contigs.fasta",outdir=OUTDIR,sample=SAMPLES),
        expand("{outdir}/result/spades/{sample}/spades_out/{sample}.amr",outdir=OUTDIR,sample=SAMPLES),
        expand("{outdir}/result/filter/kneaddata.stats",outdir=OUTDIR)
...
rule kneaddata:
    input:
        ...
        single = "{rawdata_dir}/{sample}.fq.gz"
    output:
        #directory("{outdir}/result/"),
        "{outdir}/result/filter/{sample}.fastq",
        "{outdir}/result/filter/{sample}.log"

    params:
        thread_num = 16,
        output_dir = "{outdir}/result/filter"
    shell:
        "{kneaddata} \
        -i {input.single} \
        -o {params.output_dir}  \
...
#         "{outdir}/result/filter/{sample}.fastq"
#     output:
#         "{outdir}/result/filter/{sample}.f.fastq"
#     shell:
#         "{PriceSeqFilter} \
#         -a 12 -rnf 90 -log c -f {outdir}/result/filter/{sample}.fastq \
...
rule PriceSeqFilter:
    input:
        #"{outdir}/result/filter/kneaddata.sum.txt",
        "{outdir}/result/filter/{sample}.fastq"
    output:
        "{outdir}/result/filter/{sample}.f.fastq"
    shell:
...
# rule kneaddata_stat:
#     input:
#         #expand("{outdir}/result/filter",outdir=OUTDIR,sample=SAMPLES)
#         "{outdir}/result/filter"
#         #The flag 'directory' used in rule kneaddata_stat is only valid for outputs, not inputs.
#     output:
#         kneaddata_sum = "{outdir}/result/filter/kneaddata.sum.txt"
#     shell:
...
rule kraken2_classify:
    input:
        "{outdir}/result/filter/{sample}.f.fastq"
    output:
        "{outdir}/result/kraken2/{sample}.report",
        "{outdir}/result/kraken2/{sample}.output"
    params:
        thread_num = 16
    shell:
...
rule bracken_Abundance_estimation:
    input:
        "{outdir}/result/kraken2/{sample}.report"
    output:
        "{outdir}/result/kraken2/{sample}.s.bracken",
        "{outdir}/result/kraken2/{sample}.s.bracken.report",
        "{outdir}/result/kraken2/{sample}.g.bracken",
        "{outdir}/result/kraken2/{sample}.g.bracken.report"
...
rule  bracken_format_rpm:
    ...
    output:
        ###不是变量,仅作为rule all输出
        "{outdir}/result/kraken2/{sample}.s.bracken.csv"
    shell:
...
#         "{outdir}/result/kraken2/{sample}.report"
#     output:
#         "{outdir}/result/kraken2/{sample}.report.mpa.txt"
#     shell:
...
rule  kreport2mpa:
    ...
    output:
        "{outdir}/result/kraken2/{sample}.report.mpa.txt",
        "{outdir}/result/kraken2/{sample}.report.mpa.rpm.csv"
    shell:
...
rule  kreport2krona:
    input:
        "{outdir}/result/kraken2/{sample}.report"
    output:
        "{outdir}/result/kraken2/{sample}.krona",
        "{outdir}/result/kraken2/{sample}.krona.html"
    shell:
...
#     input:
#         expand("{outdir}/result/filter/{sample}.f.fastq",outdir=OUTDIR,sample=SAMPLE)
#     output:
#         expand("{outdir}/result/align/{sample}.clean.refseq.fq.gz",outdir=OUTDIR,sample=SAMPLE),
#         expand("{outdir}/result/align/{sample}.refseq.sam",outdir=OUTDIR,sample=SAMPLE)
#     params:
#         bowtie2_bin="{bowtie2}/bowtie2",
#         threads=64,
#         bac_fungi_protozoa_viral_index="/mnt/data/bowtie2/bacteria_fungi_protozoa_viral"
#     log:
#         log_file"{outdir}/result/align/{sample}.bowtie2.refseq.log",
#     shell:
...
rule clean_reads_bowtie2_bac_fungi_protozoa_viral:
    input:
        "{outdir}/result/filter/{sample}.f.fastq"
    output:
        "{outdir}/result/align/{sample}.clean.refseq.fq.gz",
        "{outdir}/result/align/{sample}.refseq.sam"
    params:
        threads=64,
        bac_fungi_protozoa_viral_index="/mnt/data/bowtie2/bacteria_fungi_protozoa_viral"
    log:
...
rule sam2bam:
    input:
        "{outdir}/result/align/{sample}.refseq.sam"
    output:
        "{outdir}/result/align/{sample}.bam"
    shell:
...
rule samtools_sort:
    input:
        "{outdir}/result/align/{sample}.bam"
    output:
        "{outdir}/result/align/{sample}.sorted.bam"
    shell:
        "{samtools} sort -o {output} {input}"

# rule bam_index:
#     input:
#         prefix = "{outdir}/result/align/{sample}.sorted",
#         refseq_sam = "{outdir}/result/align/{sample}.refseq.sam",
#         bam="{outdir}/result/align/{sample}.bam"
#     output:
#         ###不做output ，
#         "{outdir}/result/align/{sample}.sorted.bam"
#     shell:
...
rule Covem:
    input:
        "{outdir}/result/align/{sample}.sorted.bam"
    output:
        "{outdir}/result/align/{sample}.cove.tsv"
    shell:
...
rule Covem_format:
    input:
        "{outdir}/result/align/{sample}.cove.tsv"
    output:
        "{outdir}/result/align/{sample}.refseq.cove.tsv"
    shell:
...
rule SPAdes:
    ...
    output:
        ...
        "{outdir}/result/spades/{sample}/spades_out/contigs.fasta"

    params:
        out_dir = "{outdir}/result/spades/{sample}/spades_out"
    shell:
        #SPAdes
...
rule amrfinder:
    ...
    input:
        "{outdir}/result/spades/{sample}/spades_out/contigs.fasta"
    output:
        "{outdir}/result/spades/{sample}/spades_out/{sample}.amr"
    shell:
...
rule stats:
    input:
        expand("{outdir}/result/filter/{sample}.log",outdir=OUTDIR,sample=SAMPLES)
    output:
        "{outdir}/result/filter/kneaddata.stats"

    params:
        input_dir = directory("{outdir}/result/filter")
    shell:
        "kneaddata_read_count_table --input {params.input_dir} --output {output} && rm {params.input_dir}/*yh_add_hg19_GRCh38_latest_rna_bowtie2_contam.fastq "
