...
human_database="/mnt/db/kneaddata/human_genome/hg37dec_v0.1"
...
quali_filter = Task(
    id="quali_filter",  # your task id, should be unique
    work_dir=out_dir + "/bin/" + sample,  # you task work directory
    type="local",  # the way your task run. if "sge", task will submit with qsub
    option="-step 2",  # the option of "sge" or "local"
    script="{fastp} -i {input_single} --dedup  --length_required 50 -q 15 -u 40 --thread 4 \
    --json  {outdir}/result/{sample}/filter/{sample}.json --html  {outdir}/result/{sample}/filter/{sample}.html -o {outdir}/result/{sample}/filter/{sample}.clean.fq.gz".format(fastp=fastp, input_single=fq,outdir=out_dir,sample = sample)
)


host_remove = Task(
    id="host_remove",  # your task id, should be unique
    work_dir=out_dir + "/bin/" + sample,  # you task work directory
    type="local",  # the way your task run. if "sge", task will submit with qsub
    option="-step 3",  # the option of "sge" or "local"
    script = "{bowtie2} --mm --very-sensitive \
    -x {human_database} \
    ...
    1> {outdir}/result/{sample}/filter/{sample}.bowtie.log 2>&1".format(bowtie2=bowtie2_bin,human_database=human_database,outdir=out_dir,sample=sample)
)
...
kraken2_classify = Task(
    ...
    script = "{KRAKEN} --db {db} {outdir}/result/{sample}/filter/{sample}.filter.fq \
        --threads 16 \
...
bracken_Abundance_estimation = Task(
    ...
    script = "{Bracken} -d {db} -i {outdir}/result/{sample}/kraken2/{sample}.report \
        ...
        -w {outdir}/result/{sample}/kraken2/{sample}.s.bracken.report -r 50 -l S \
        && {Bracken} -d {db} -i {outdir}/result/{sample}/kraken2/{sample}.report \
        -o {outdir}/result/{sample}/kraken2/{sample}.g.bracken \
        -w {outdir}/result/{sample}/kraken2/{sample}.g.bracken.report -r 50 -l G"
...
bracken_format_rpm = Task(
    ...
    type="local",  # the way your task run. if "sge", task will submit with qsub
    option="-step 5",  # the option of "sge" or "local"
    script = "python  ~/idseq_wkf/scripts/bracken_reads_rpm.py -w {outdir}  -p {sample} -c {outdir}/result/{sample}/filter/{sample}.filter.fq".format(outdir=out_dir,sample=sample)
)
...
kreport2mpa2rpm_BSD = Task(
    ...
    script = """
            python  ~/idseq_wkf/KrakenTools/kreport2mpa.py --no-intermediate-ranks --read_count -r {outdir}/result/{sample}/kraken2/{sample}.uniq_kreport -o {outdir}/result/{sample}/kraken2/{sample}.uniq_kreport.mpa.txt 
            python  ~/idseq_wkf/KrakenTools/kreport2mpa.py --no-intermediate-ranks --read_count -r {outdir}/result/{sample}/kraken2/{sample}.uniq80_kreport -o {outdir}/result/{sample}/kraken2/{sample}.uniq80_kreport.mpa.txt
            python  ~/idseq_wkf/scripts/kraken_reads_rpm_bsd_2.py {outdir}/result/{sample}/filter/{sample}.filter.fq {outdir}/result/{sample}/kraken2/{sample}.kreport.mpa.txt {outdir}/result/{sample}/kraken2/{sample}.kreport.mpa.txt {outdir} {sample} &
            python  ~/idseq_wkf/scripts/kraken_reads_rpm_bsd_2.py {outdir}/result/{sample}/filter/{sample}.filter.fq {outdir}/result/{sample}/kraken2/{sample}.kreport.mpa.txt {outdir}/result/{sample}/kraken2/{sample}.uniq80_kreport.mpa.txt {outdir} {sample} &
            python  ~/idseq_wkf/scripts/kraken_reads_rpm_bsd_2.py {outdir}/result/{sample}/filter/{sample}.filter.fq {outdir}/result/{sample}/kraken2/{sample}.kreport.mpa.txt {outdir}/result/{sample}/kraken2/{sample}.uniq_kreport.mpa.txt {outdir} {sample} &
    """.format(outdir = out_dir,sample=sample)
)
...
clean_reads_bowtie2_bac_fungi_protozoa_viral = Task(
    ...
    -U {outdir}/result/{sample}/filter/{sample}.filter.fq --al-gz {outdir}/result/{sample}/align/{sample}.clean.refseq.fq.gz -S {outdir}/result/{sample}/align/{sample}.refseq.sam \
    1>{outdir}/result/{sample}/align/{sample}.bowtie2.refseq.log 2>&1".format(bowtie2_bin = bowtie2_bin,threads=64,bac_fungi_protozoa_viral_index=bac_fungi_protozoa_viral_index,outdir=out_dir,sample=sample)
)
...
def run_negative_control():
    ...
    wkf1.add_task(mkdir)
    wkf1.add_task(quali_filter)
    quali_filter.set_downstream(host_remove)
    wkf1.add_task(host_remove)
    host_remove.set_downstream(kraken2_classify)
