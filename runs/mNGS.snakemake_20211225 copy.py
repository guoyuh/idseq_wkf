...
rule make_kreport:
    ...
    shell:
        """
            python ~/idseq_wkf/KrakenTools/make_kreport.py -i {input[0]} -t /mnt/data/kraken2_mask/taxonomy/mydb_taxonomy.txt -o {output[0]}
            python ~/idseq_wkf/KrakenTools/make_kreport.py -i {input[1]} -t /mnt/data/kraken2_mask/taxonomy/mydb_taxonomy.txt -o {output[1]}
        """
...
rule Covem:
    ...
    shell:
        "{Coverm} genome -m covered_bases covered_fraction variance length count rpkm --min-covered-fraction 0 --bam-files {input} --genome-fasta-directory {refsesq_dir} --genome-fasta-extension fna > {output} "
