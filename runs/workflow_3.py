...
bracken_format_rpm = Task(
    ...
    type="local",  # the way your task run. if "sge", task will submit with qsub
    option="-step 5",  # the option of "sge" or "local"
    script = "python  ~/idseq_wkf/scripts/bracken_reads_rpm.py -w {outdir}  -p {sample} -c {outdir}/result/{sample}/filter/{sample}.f.fastq".format(outdir=out_dir,sample=sample)
)
...
kreport2mpa2rpm_BSD = Task(
    ...
    script = """
            python  ~/idseq_wkf/KrakenTools/kreport2mpa.py --no-intermediate-ranks --read_count -r {outdir}/result/{sample}/kraken2/{sample}.uniq_kreport -o {outdir}/result/{sample}/kraken2/{sample}.uniq_kreport.mpa.txt 
            python  ~/idseq_wkf/KrakenTools/kreport2mpa.py --no-intermediate-ranks --read_count -r {outdir}/result/{sample}/kraken2/{sample}.uniq80_kreport -o {outdir}/result/{sample}/kraken2/{sample}.uniq80_kreport.mpa.txt
            python  ~/idseq_wkf/scripts/kraken_reads_rpm_bsd.py {outdir}/result/{sample}/filter/kneaddata.sum.txt {outdir}/result/{sample}/kraken2/{sample}.kreport.mpa.txt {outdir}/result/{sample}/kraken2/{sample}.kreport.mpa.txt {outdir} {sample} &
            python  ~/idseq_wkf/scripts/kraken_reads_rpm_bsd.py {outdir}/result/{sample}/filter/kneaddata.sum.txt {outdir}/result/{sample}/kraken2/{sample}.kreport.mpa.txt {outdir}/result/{sample}/kraken2/{sample}.uniq80_kreport.mpa.txt {outdir} {sample} &
            python  ~/idseq_wkf/scripts/kraken_reads_rpm_bsd.py {outdir}/result/{sample}/filter/kneaddata.sum.txt {outdir}/result/{sample}/kraken2/{sample}.kreport.mpa.txt {outdir}/result/{sample}/kraken2/{sample}.uniq_kreport.mpa.txt {outdir} {sample} &
    """.format(outdir = out_dir,sample=sample)
)
...
clean_reads_bowtie2_bac_fungi_protozoa_viral = Task(
    ...
    -U {outdir}/result/{sample}/filter/{sample}.f.fastq --al-gz {outdir}/result/{sample}/align/{sample}.clean.refseq.fq.gz -S {outdir}/result/{sample}/align/{sample}.refseq.sam \
